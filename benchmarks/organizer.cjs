...
if (!revision) { throw new Error('Pass the baseline commit to compare.'); }
const source = execFileSync('git', ['show', `${revision}:src/lib/organizer.ts`], { encoding: 'utf8' });
...
for (const count of [100, 1000, 5000]) {
	for (const scenario of ['distinct', 'blocks', 'defaults']) {
		const input = Array.from({ length: count }, (_, i) => scenario === 'defaults'
		...
		for (let i = 0; i < 4; i++) { baseline.organizeImportsContent(input); current.organizeImportsContent(input); }
		const times = [[], []];
		for (let i = 0; i < 11; i++) {
		...
		console.log(JSON.stringify({ scenario, count, beforeMs: +before.toFixed(2), afterMs: +after.toFixed(2), improvementPercent: +((1 - after / before) * 100).toFixed(1) }));
	}
}
