...
import {
	getImportFixes,
	getNamespaceImportFixes,
	organizeImportsWithReport,
	removeUnusedImportsByScan
} from './lib/organizer';
...
class LiveImportDiagnostics implements vscode.Disposable {
		...
		this.subscriptions = [
			...
			vscode.commands.registerCommand(COMMAND_APPLY_FIX, async (uri: vscode.Uri, version: number, index: number, generation: number) => {
				...
				if (!await vscode.workspace.applyEdit(edit)) {
					void vscode.window.showErrorMessage('Import Authority failed to apply the quick fix.');
				}
			}),
		];
		this.invalidateAll();
	...
	public actions(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction[] {
		...
		return this.refresh(document).flatMap((fix, index) => {
			if (fix.end < start || fix.start > end) {
				return [];
			}
			...
			action.command = {
				command:   COMMAND_APPLY_FIX,
				title:     fix.title,
				arguments: [document.uri, document.version, index, this.snapshots.get(document.uri.toString())!.generation]
			};
...
class ImportAuthorityCodeActionProvider implements vscode.CodeActionProvider {
	...
	public constructor(private readonly live: LiveImportDiagnostics) {
	}
	...
	public provideCodeActions(document: vscode.TextDocument, _range: vscode.Range, context: vscode.CodeActionContext): vscode.CodeAction[] {
		...
		if (_range.start !== undefined && _range.end !== undefined
			...
			for (const fix of analyzeNamespaceImports(document).fixes) {
				...
				action.command = {
					command:   COMMAND_CONVERT_NAMESPACE,
					title:     action.title,
					arguments: [document.uri, document.version, fix.start, fix.name]
				};
				fixes.push(action);
			}
		}

		if (context.only && !context.only.contains(ImportAuthorityCodeActionProvider.kind)) {
			return fixes;
		}
...
function readAliasPrefixesFromConfig(configPath: string): string[] {
	try {
		...
		if (!parsed) {
			aliasPrefixCache.set(configPath, {
				mtimeMs,
				prefixes: []
			});
...
function getOptions(document: vscode.TextDocument): ExtensionOptions {
	...
	if (configPath) {
		readAliasPrefixesFromConfig(configPath);
	}
...
async function computeOrganizedContent(document: vscode.TextDocument): Promise<OrganizedContent | undefined> {
	for (let attempt = 0; attempt < 2; attempt += 1) {
		...
		if (initialReport.importCount > 0) {
			report.reasons = report.reasons.filter(reason => reason !== 'no-imports');
		}
		...
		return {
			...
			organized,
			version,
			summary: describeOrganization(report, organized !== original, removed, notes),
...
function analyzeNamespaceImports(document: vscode.TextDocument): NamespaceConversionResult {
	...
	const openFiles = new Map(vscode.workspace.textDocuments.filter(doc => doc.uri.scheme === 'file').map(doc => [path.resolve(doc.uri.fsPath), doc.getText()]));
...
async function convertNamespaceImport(targetUri?: vscode.Uri, expectedVersion?: number, importStart?: number, importName?: string): Promise<void> {
	...
	if (expectedVersion !== undefined && expectedVersion !== version) {
		void vscode.window.showInformationMessage('The document changed. Request namespace conversion again.');
		return;
	}
	...
	if (!fix && importStart === undefined && result.fixes.length > 1) {
		const selected = await vscode.window.showQuickPick(result.fixes.map(candidate => ({
			label:       candidate.name,
			description: candidate.moduleName,
			fix:         candidate
		})), {
	...
	if (!await vscode.workspace.applyEdit(edit)) {
		void vscode.window.showErrorMessage('Import Authority failed to convert the namespace import.');
	}
}
