...
function receiverIndependent(access: ts.PropertyAccessExpression, checker: ts.TypeChecker): boolean {
	...
	for (const declaration of symbol?.declarations ?? []) {
		...
		if (fn.parameters.some(parameter => ts.isIdentifier(parameter.name) && parameter.name.text === 'this')) {
			continue;
		}
		...
		const inspectModule = (node: ts.Node): void => {
			if (ts.isIdentifier(node) && (node.text === 'eval' || checker.getSymbolAtLocation(node) === symbol && isWrite(node))) {
				dependent = true;
			}
...
function isReceiverCall(access: ts.Node): boolean {
	...
	while (ts.isParenthesizedExpression(expression.parent) || ts.isNonNullExpression(expression.parent)
		|| ts.isAsExpression(expression.parent) || ts.isTypeAssertionExpression(expression.parent) || ts.isSatisfiesExpression(expression.parent)) {
...
export function planNamespaceImports(source: ts.SourceFile, imports: ts.ImportDeclaration[], context: NamespaceConversionContext = {}): NamespaceConversionResult {
	...
	if (context.directoryExists) {
		host.directoryExists = context.directoryExists;
	}
	...
	const identifiers     = [] as ts.Identifier[];
	...
	for (const comment of ts.getLeadingCommentRanges(source.text, 0) ?? []) {
		for (const match of source.text.slice(comment.pos, comment.end).matchAll(/@jsx(?:Frag)?\s+([\w$]+)/g)) {
	...
	for (const statement of candidates) {
		...
		if (documentedNames.has(name)) {
			skip('JSDoc references still need the namespace.');
			continue;
		}

		if (hasEval || hasJsx && factories.has(name)) {
			skip('eval or an implicit JSX factory may use the namespace.');
			continue;
		}

		if (statement.attributes || clause.phaseModifier === ts.SyntaxKind.DeferKeyword || hasComments(statement.getText(source))) {
			skip('The import contains comments, attributes, or a deferred binding.');
			continue;
		}

		const symbol = checker.getSymbolAtLocation(namespace.name);
		if (!symbol) {
			skip('The namespace binding could not be resolved.');
			continue;
		}

		const module = checker.getAliasedSymbol(symbol);
		if (module.exports?.has('export=' as ts.__String)) {
			skip('The module uses export = rather than named exports.');
			continue;
		}

		const exportedNames = new Set(checker.getExportsOfModule(module).map(exported => exported.name));
		const accesses      = [] as Array<ts.PropertyAccessExpression | ts.QualifiedName>;
		let reason: string | undefined;

		for (const identifier of identifiers) {
			if (identifier === namespace.name) {
				continue;
			}
			...
			if (!(ts.isPropertyAccessExpression(access) && access.expression === identifier || ts.isQualifiedName(access) && access.left === identifier)) {
				reason = 'The namespace is used as an object or through computed access.';
				break;
			}

			if (ts.isPropertyAccessExpression(access) && (access.questionDotToken || isWrite(access))) {
				reason = 'The namespace has optional access or writes.';
				break;
			}

			const member = ts.isPropertyAccessExpression(access) ? access.name : access.right;
			if (!exportedNames.has(member.text)) {
				reason = 'A named export could not be resolved in the imported module.';
				break;
			}
			...
			if (!ts.isIdentifier(member) || member.text === 'default' || hasComments(access.getText(source))
				...
				reason = 'A member access has unsupported syntax, comments, or default interop.';
				break;
			}

			if (ts.isPropertyAccessExpression(access) && isReceiverCall(access) && !receiverIndependent(access, checker)) {
				reason = 'A called export may depend on its namespace receiver, or its implementation is unavailable.';
				break;
			}
		...
		if (reason || accesses.length === 0) {
			skip(reason ?? 'The namespace has no convertible member references.');
			continue;
		}
		...
		for (const access of accesses) {
			...
			let local     = base;
			for (let suffix = 1; reserved.has(local); suffix += 1) {
				local = `${name}_${base}${suffix > 1 ? suffix : ''}`;
			}

			reserved.add(local);
			names.set(member, local);
		}

		const specifiers = [...names].map(([member, local]) => member === local ? member : `${member} as ${local}`);
		const edits = [{
			start:   namespace.getStart(source),
			end:     namespace.getEnd(),
			newText: `{ ${specifiers.join(', ')} }`
		}] as OffsetEdit[];

		for (const access of accesses) {
			const member = (ts.isPropertyAccessExpression(access) ? access.name : access.right).text;
			edits.push({
				start:   access.getStart(source),
				end:     access.getEnd(),
				newText: names.get(member)!
			});
		}

		result.fixes.push({
			name,
			moduleName: (statement.moduleSpecifier as ts.StringLiteral).text,
