...
test('converts type-only uses by default before sorting and merging', () => {
	...
	assert.doesNotMatch(organizeImportsContent(content, 'a.ts', { convertTypeOnlyImports: false }), /import type \{ Model/);
});
...
test('converts default, namespace, aliased and string-named bindings into legal declarations', () => {
	for (const content of [
		...
		const diagnostics = ts.transpileModule(report.content, {
			compilerOptions: { module: ts.ModuleKind.ESNext, verbatimModuleSyntax: true }, reportDiagnostics: true,
		}).diagnostics ?? [];
...
test('preserves runtime JSX factories while converting unrelated TSX types', () => {
	...
	assert.equal(organizeImportsWithReport(custom, 'a.tsx', { jsxFactory: 'h', jsxFragmentFactory: 'Frag' }).converted, 1);
...
test('respects inline style, preserves comments and keeps explicit side-effect imports', () => {
	...
	const inline = organizeImportsWithReport(content, 'a.ts', { typeImportStyle: 'inline' });
